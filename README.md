...
The drafting and critic roles can each use a real local Pi RPC process. Research, review, and deep review remain mocked in v0.
...
The response contains both `"draftingEngine":"pi-rpc"` and `"criticEngine":"pi-rpc"`. Drafting prompts ask for a short whimsical email written like a medieval knight and require raw JSON matching `DraftResultSchema`.
...
- Research, review, and deep review remain mocked. Drafting and critic are selected independently with `DRAFTING_ENGINE=mock|pi` and `CRITIC_ENGINE=mock|pi`.
- No email is sent.
